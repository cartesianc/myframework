...
module MyFramework.Handler.Internal
  ( CommandHandler
  , CommitState (..)
  , CudeInvocation (..)
  , ExecutionStatus (..)
...
invokeCude ::
  ...
  RuntimeData ->
  IO CudeInvocation
invokeCude currentPermit currentRegistry currentId currentInput currentArguments =
  executionPermitProvenance currentPermit `seq`
...
invokeRead ::
  ...
  HandlerInput ->
  IO ReadInvocation
invokeRead currentPermit currentRegistry currentId currentInput =
  executionPermitProvenance currentPermit `seq`
